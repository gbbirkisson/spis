...
#[get("/favorite")]
async fn favorite(pool: Data<Pool<Sqlite>>, state: Query<State>) -> Response {
...
#[get("/year/{year}")]
async fn year(pool: Data<Pool<Sqlite>>, state: Query<State>, path: Path<usize>) -> Response {
    let mut state = state.into_inner();
...
#[get("/month/{month}")]
async fn month(pool: Data<Pool<Sqlite>>, state: Query<State>, path: Path<u8>) -> Response {
    let mut state = state.into_inner();
...
#[get("/bar/clear")]
async fn clear(pool: Data<Pool<Sqlite>>) -> Response {
